...
// Ported from SiteLoader.module.scss (#logo animations). Colors are the theme
// tokens gold (#98821e) and gray (#8E8E8E) inlined — keyframes are plain
// emotion templates and can't resolve Chakra token names.
// Note: the legacy `display: block/none` steps were non-animatable no-ops and
// are omitted; opacity + transform carry the visible scale-out.
const animateLine = keyframes`
    60% { stroke-dashoffset: 0; fill: transparent; }
    70% { stroke: #98821e; fill: #98821e; stroke-dashoffset: 0; }
    100% { fill: #8E8E8E; stroke: #8E8E8E; stroke-dashoffset: 0; }
`;
const animateScale = keyframes`
    from { transform: translate(-50%, -50%) scale(1); opacity: 1; }
    to { transform: translate(-50%, -50%) scale(250); opacity: 0; }
`;
    ...
    return (
		...
		>
            <Box
                ...
                overflow="hidden"
                background="bg-main"
                boxShadow="inset 0px 0px 300px 30px var(--chakra-colors-shadow-main)"
            >
                <chakra.svg
                    width="252" height="159" viewBox="0 0 252 159" fill="none" xmlns="http://www.w3.org/2000/svg"
                    position="absolute"
                    top="50%"
                    left="50%"
                    transform="translate(-50%, -50%)"
                    animation={`${animateScale} 2s forwards 2s`}
                    sx={{
                        "& path:nth-of-type(1)": {
                            stroke: "#98821e",
                            strokeDasharray: 1281,
                            strokeDashoffset: 1281,
